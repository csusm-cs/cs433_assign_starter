 ...
 * @brief This file tests the correctness of your implementation of the
 * readyqueue and pcbtable.
 */
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Description: Program to implement a priority ready queue of "
               "processes"
            << std::endl;
  ...
  std::cout << "add processes 47, 1, 37 and 5 into q1 and display q1"
            << std::endl;
  ...
  std::cout << "Insert processes 47, 17, 39, 12 and 19  to q1and display q1."
            << std::endl;
  q1.addPCB(table.getPCB(47));
  ...
  q1.displayAll();
  std::cout << "One by one remove the process with the highest priority from "
               "the queue q1"
            << std::endl;
  while (q1.size() > 0) {
    p = q1.removePCB();
