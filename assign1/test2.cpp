...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 1000000; i++) {
    ...
    if (x % 2 == 0) {
      ...
      if (table.getPCB(idx)->getState() !=
          ProcState::READY) { // if the PCB is not in READY state, add it into
                              // ReadyQueue
        int priority = rand() % 50 + 1;
        table.getPCB(idx)->setPriority(
            priority); // change its priority to a random value
  ...
  cout << "# of removes = " << remove_count
       << ", # of inserts = " << insert_count << endl;
  q2.displayAll();
}
